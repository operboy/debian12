...
# 检查依赖工具
REQUIRED_TOOLS=("ipcalc" "curl" "awk")
for tool in "${REQUIRED_TOOLS[@]}"; do
    if ! command -v $tool &>/dev/null; then
        echo "错误: 依赖工具 $tool 未安装，请先安装该工具后重试"
        exit 1
    fi
done
...
get_current_network_info() {
    ...
    CURRENT_NETMASK=$(ipcalc -m "$CURRENT_IP/$CURRENT_CIDR" | awk -F': ' '{print $2}')
...
get_all_network_info() {
    ...
    echo -e "${BLUE}# Debian 网络配置命令（复制以下全部内容）：${NC}"
